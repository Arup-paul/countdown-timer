<template>
    <div v-if="loaded">
        ...
        <div class="days mr-2 relative">
            {{displayDays}}
            <div class="label text-sm absoulte bottom-0">days</div>
        </div>
        <span>:</span>
        <div class="hours mx-2 relative">
            {{displayHours}}
            <div class="label text-sm absoulte bottom-0">hours</div>
        </div>
        <span>:</span>
        <div class="minutes mx-2 relative">
            {{displayMinutes}}
            <div class="label text-sm absoulte bottom-0">minute</div>
        </div>
        <span>:</span>
        <div class="seconds ml-2 relative">
            {{displaySeconds}}
            <div class="label text-sm absoulte bottom-0">Second</div>
        </div>
        </section>
    </div>
</template>
    ...
    methods:{
        ...
        showRemaining(){
            const timer = setInterval(() => {
                const now  = new Date();
                // const end = new Date(2021,2,22,10,10,10);
